...
### 기능 5: 페이지네이션 및 검색
- 게시글 목록: cursor-based 페이지네이션 (기본) + offset 기반 페이지네이션 (`page` 파라미터 사용 시)
- 제목/내용 키워드 검색
- 정렬 옵션: 최신순, 조회순
...
#### 게시글
- `GET /api/v1/posts` — 목록 조회 (cursor/offset 페이지네이션, 검색, `page` 파라미터로 offset 모드 전환)
...
| 단계 | 내용 | 상태 |
|------|------|------|
| M1 | 프로젝트 초기 세팅 (NestJS + Prisma + 공통 모듈) | 완료 |
| M2 | 사용자 인증 (회원가입/로그인 JWT) | 완료 |
| M3 | 게시글 CRUD + 조회수 | 완료 |
| M4 | 댓글 CRUD | 완료 |
| M5 | 파일 업로드 (S3 Presigned URL) | 완료 |
| M6 | 페이지네이션 + 검색 (cursor + offset) | 완료 |
